...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    num_of_typed_words = len(typed_words)
    num_of_reference_words = len(reference_words)

    if num_of_typed_words == 0:
        return 0.0 if num_of_reference_words > 0 else 100.0
    else:
        num_of_same_words = 0
        for idx, word in enumerate(reference_words):
            if idx == num_of_typed_words:
                break
            if word == typed_words[idx]:
                num_of_same_words += 1
        
        return num_of_same_words / num_of_typed_words * 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return autocorrect_word if lowest_diff <= limit else typed_word
    # END PROBLEM 5
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    correct_num = 0
    prompt_words_num = len(prompt)

    for idx, word in enumerate(sofar):
        if word == prompt[idx]:
            correct_num += 1
        else:
            break
    
    progress = correct_num / prompt_words_num
...
def time_per_word(words, times_per_player):
    ...
    for times in times_per_player:
        match['times'].append([end_time - start_time for start_time, end_time in zip(times[:-1], times[1:])])
...
def fastest_words(match):
    ...
    for word_indice in word_indices:
        ...
        for player_indice in player_indices:
            player_time = match['times'][player_indice][word_indice]
            if player_time < fastest_time:
                fastest_time = player_time
                fastest_player = player_indice
        
        fastest_words_list[fastest_player].append(word_at(match, word_indice))
