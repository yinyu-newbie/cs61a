def repeated(t, k):
    ...
    while True:
        ...
        if current_value == prev_value and count == k - 1:
            return current_value
        elif current_value == prev_value and count != k - 1:
            count += 1
        else:
            count = 1
...
def merge(incr_a, incr_b):
    ...
    while next_a != None or next_b != None:
        if next_a == None or next_b < next_a:
            yield next_b
...
def add_d_leaves(t, v):
    ...
    def helper(t, v, depth):
        # if t.is_leaf():
        #     t.branches.extend(depth * [Tree(v)])
        #     return
...
def shuffle(cards):
    ...
    for i in range(len(cards)//2):
        shuffled.append(half[0][i])
        shuffled.append(half[1][i])
    return shuffled
...
class PairsIterator:
    ...
    def __next__(self):
        "*** YOUR CODE HERE ***"
        if self.pos_x == self.len:
            raise StopIteration

        x, y = lst(self.pos_x), lst(self.pos_y)

        if self.pos_y == self.len - 1:
            self.pos_x += 1
            self.pos_y = 0
        else:
            self.pos_y += 1
