...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            ant_contained = place.ant.can_contain(self)
            self_contained = self.can_contain(place.ant)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        range = 0
        while place.is_hive == False:
            if place.bees and self.min_range <= range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            amount += self.damage
            super().reduce_health(self.health)
        else:
...
# The WallAnt class
class WallAnt(Ant):
    "WallAnt protect our glorious home!"
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        return True if self.ant_contained is None and other.is_container == False else False
        # END Problem 8
...
class ScubaThrower(ThrowerAnt):
    """Thrower ant that can be placed on water"""
    name = 'Scuba'
    food_cost = 6
    is_waterproof = True
    implemented = True
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        last_place = self.place.exit

        while last_place is not None:
            ant = last_place.ant
            if ant is not None:
                if ant.double_damage == False:
                    ant.damage = ant.damage * 2
                    ant.double_damage = True
                if ant.is_container:
                    ant_contained = ant.ant_contained
                    if ant_contained is not None and ant_contained.double_damage == False:
                        ant_contained.damage  = ant_contained.damage * 2
                        ant_contained.double_damage = True

            last_place = last_place.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # when bee is slow
        if self.left_turn_at_slow != 0:
            self.left_turn_at_slow -= 1
            if gamestate.time % 2 == 1:
                destination = self.place

        # when bee is scary
        if self.left_turn_at_scary != 0:
            if self.left_turn_at_slow == 0 or gamestate.time % 2 == 0:
                self.left_turn_at_scary -= 1
                if self.place.entrance != None and self.place.entrance.is_hive == False:
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.had_scared == False:
            self.had_scared = True
            self.left_turn_at_scary += length
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        place = self.place
        while place is not None and place.is_hive == False:
            ant = place.ant if place.ant is not None and place.ant.name != 'Laser' else None
            bees = place.bees[:]
            ant_contained = ant.ant_contained if ant is not None and ant.is_container and ant.ant_contained is not None else None
            ...
            if ant_contained is not None:
                insects_and_distances[ant_contained] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        initial_damage = 2 - 0.0625 * self.insects_shot
        shot_damage = initial_damage - 0.25 * distance
        if shot_damage > 0:
            return shot_damage
        else:
            return 0
